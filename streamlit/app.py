...
with k4:
    ...
    wn = worst['zone_name'] if worst is not None else "—"
    wl = worst['estimated_revenue_loss_usd'] if worst is not None else 0
    st.markdown(f'<div class="kpi-card"><div class="kpi-label">⚠️ Worst Zone</div><div class="kpi-value" style="font-size:18px;">{wn}</div><div class="kpi-delta down">${wl/1e6:.1f}M lost</div></div>', unsafe_allow_html=True)
with k5:
...
with insight_col:
    ...
    st.markdown('<div class="finding-item low"><div class="finding-title">Marble Hill Anomaly</div><div class="finding-desc">Labeled Manhattan, spatially within Bronx. Administrative vs geographic boundary mismatch (Harlem Ship Canal, 1895). Revenue impact: $0.</div><span class="finding-badge low">Informational</span></div>', unsafe_allow_html=True)
...
### Map Views
- **2D Overview**: all {len(df)} zones colored by status (red/yellow/blue)
- **3D Revenue Loss**: only undersupplied zones, column height = revenue loss magnitude
...
display = filtered[['zone_name','borough','zone_status','total_demand_trips','total_supply_trips','supply_demand_ratio','unmet_demand_rate','estimated_revenue_loss_usd']].copy()
display.columns = ['Zone','Borough','Status','Demand (trips)','Supply (trips)','S/D Ratio','Unmet Rate','Revenue Loss (USD)']
